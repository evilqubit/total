...
$dbs = "SELECT * FROM participants WHERE status = '2'";
$extractx = mysql_query ($dbs);
$numrows = mysql_num_rows ($extractx);


$db = "SELECT * FROM participants WHERE status = '2' ORDER BY id DESC LIMIT $i, 50";
...
?>
		...
		<div class="tab_container">
			...
			?>
				<tr>
    				...
    				<td><input onClick="window.location='disapproved_d.php?user=<?php echo $row['id'];?>'" type="image" src="images/icn_edit.png" title="Details">
                    
                    <form method="post" action="disapproved.php">
                    <input type="hidden" value="<?php echo $row['id'];?>" name="user">
                    <input name="remove_vote" type="submit" value="" style="background-image:url(images/icn_trash.png); background-repeat:no-repeat; background-color:transparent; border:none; width:16px; height:14px;" src="images/icn_trash.png" title="Trash" onClick="if (confirm('Do you want to delete this user?') == false) return false; ">
                    </form></td>
				</tr> 
			<?php
			}
	...
	if ($y == 1)
	{
		...
		while ( ($count < $lim) && ($y < $x) )
		{
			...
			$y += 1;
			$count += 1;?>
		<form name="poll_page" action="disapproved.php" method="post">
