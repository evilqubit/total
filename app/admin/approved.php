...
$dbs = "SELECT * FROM participants WHERE status = '1'";
$extractx = mysql_query ($dbs);
$numrows = mysql_num_rows ($extractx);


$db = "SELECT * FROM participants WHERE status = '1' ORDER BY id DESC LIMIT $i, 50";
...
?>
		...
		<div class="tab_container">
			...
			?>
				<tr>
    				...
    				<td><input onClick="window.location='approved_d.php?user=<?php echo $row['id'];?>'" type="image" src="images/icn_edit.png" title="Details">
	...
	if ($y == 1)
	{
		...
		while ( ($count < $lim) && ($y < $x) )
		{
			...
			$y += 1;
			$count += 1;?>
		<form name="poll_page" action="approved.php" method="post">
